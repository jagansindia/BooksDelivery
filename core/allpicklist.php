...
$query=mysqli_query($conn,"SELECT pd.name as product_name,pr.model as author,pr.image,manuf.name as manufacturer_name,sp.quantity as pick_quantity, sp.order_product_id, sp.order_id, sp.picked, sp.picked_time FROM seller_picklist sp LEFT JOIN product pr ON sp.product_id=pr.product_id JOIN product_description pd ON pr.product_id=pd.product_id  JOIN manufacturer manuf ON manuf.manufacturer_id=pr.manufacturer_id JOIN `order` ordr ON sp.order_id=ordr.order_id where ordr.order_status_id='1' or ordr.order_status_id='2' or ordr.order_status_id='17' or ordr.order_status_id='16'");
